...
class CroppingUI(object):
    def __init__(self, pitch, width=640, height=480):
        ...
        keys = ['outline', 'Zone_0', 'Zone_1', 'Zone_2', 'Zone_3', 'Zone_4', 'Zone_5']
    ...
    def run(self, camera=False):
        ...
        if camera:
            # Open camera at index 0 (default)
            cap = cv2.VideoCapture(0)
            for i in range(10):
                status, image = cap.read()
                assert status, "Error reading from camera"
            cap.release()
        else:
    ...
    def get_pitch_outline(self):
        ...
        """
        self.get_zone('outline', 'Draw the outline of the pitch. Continue by pressing \'q\'')

import cv2
import tools
from tracker import RobotTracker,BallTracker, CircleTracker
...
frame1 =  cv2.GaussianBlur(frame0, (13, 13), 0)
...
while True:
    ...
    #frame0 = frame0[30:460, 45:610]
    positions = vision._run_trackers(frame0)
    cv2.imwrite("reshaped.jpg", frame0) 

    if positions[4] is None or positions[5] is None :
        pass
    else:
        x_ball = positions[4]['x']
        y_ball = positions[4]['y']
        # BGR colors in tuples
        cv2.imshow('frame2', cv2.circle(frame0, (int(x_ball), int(y_ball)), 8, (0, 0, 255), 2, 0))


        for crc in list(positions[5]['coordinates']):
            cv2.imshow('frame2', cv2.circle(frame0, (int(crc[0]), int(crc[1])), int(crc[2]), (0, 255, 0), 2, 0))
        """
